...
const ARGCONFIGMAP = "-cfm";
const ARGCRON = "-cron";

const DEFAULTCURLDOCKERIMAGE = "curlimages/curl:7.81.0";
...
module.exports = {
  ...
  genSecrets: (document, cmdarguments) => {
    return new Promise(async (resolv, reject) => {
      if (
        ...
        await module.exports
          .genSecretsFromEnv(document, cmdarguments)
          .then((result) => {
            yamldocument.push(result);
          })
          .catch((error) => {
   ...
   * @returns {Promise}
   */
  genSecretsFromEnv: async (document, cmdarguments) => {
    return new Promise((resolv, reject) => {
      ...
      document.varsassecret = helperGenLib.setEnvVarsAsKeyValue(document);
      if (document.varsassecret.length > 0) {
        module.exports
    ...
    return new Promise(async (resolv, reject) => {
      if (
        ...
        const persistentvolumes =
          helperGenLib.getDocumentPersistentVolumes(document);
        ...
        if (finalpersistenvolumes.length > 0) {
          ...
          for (let item of finalpersistenvolumes) {
            item.readOnly = false;
            item.access = helperGenLib.getaccessModes(item.access, scale);
            if (item.access && item.access.match(/^ReadOnly/))
              item.readOnly = true;
            const volumedata = {
   ...
   * @returns {Promise}
   */
  genConfigMaps: async (document, cmdarguments) => {
    return new Promise(async (resolv, reject) => {
      if (
        (cmdarguments.generationoptions.length === 0 ||
          cmdarguments.generationoptions.includes(ARGCONFIGMAP)) &&
        document.global &&
        document.global.configmaps
      ) {
        let template = helperGenLib.getTemplate(
          cmdarguments ? cmdarguments.customtpldir : "",
          "tpl_configmap"
        ...
        // work on copy because some attributs are altered in this process
        for (let map of JSON.parse(
          JSON.stringify(document.global.configmaps)
        )) {
          // configmap type file
          if (map.srctype === "file") {
            map.filename = path.basename(map.mount);
            map.name = helperGenLib.getConfigMapName(document, map);
            // existence check performed by the check if error delete between check and here :)
            map.content = fs.readFileSync(map.srcpath, "utf-8").split("\n");
            await module.exports
              .genManifest(template, {
                namespace: document.namespace,
                map: map,
  ...
  /**
   * see kubernetes documentation https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/
   * @param {object} document
   * @param {object} cmdarguments
   * @returns {Promise}
   */
  genCronJob: async (document, cmdarguments) => {
    return new Promise(async (resolv, reject) => {
      if (
        (cmdarguments.generationoptions.length === 0 ||
          cmdarguments.generationoptions.includes(ARGCRON)) &&
        document.global &&
        document.global.cronjob
      ) {
        let template = helperGenLib.getTemplate(
          cmdarguments ? cmdarguments.customtpldir : "",
          "tpl_cronjob"
        ...
        // work on copy because some attributs are altered in this process
        for (let cron of JSON.parse(JSON.stringify(document.global.cronjob))) {
          // configmap type file
          if (cron.type === "curl") {
            // set defaults only image set by constante (to review) others in template
            cron.image = cron.image || DEFAULTCURLDOCKERIMAGE;
            await module.exports
              .genManifest(template, {
                namespace: document.namespace,
                cron: cron,
