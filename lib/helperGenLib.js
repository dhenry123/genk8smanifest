...
module.exports = {
  ...
  /**
   * standard name for custom env container
   * @param {string} containername
   * @param {string} customenvcontainerkey
   * @returns
   */
  getSecretCustomEnvContainerName: (containername, customenvcontainerkey) => {
    return `CUSTOMCONTAINER_${containername}_CUSTOMCONTAINER_${customenvcontainerkey}`;
  },
  setEnvVarsForContainerAsKeyValue: (document, containername) => {
    // get global.env
    const containerenvvars = [];
    const globalenv = module.exports.setEnvVarsAsKeyValue(document, false);
    Object.keys(globalenv).forEach((item) => {
      containerenvvars.push({ ...globalenv[item], name: globalenv[item].key });
    });
    document.containers.forEach((container) => {
      if (container.name === containername && container.env) {
        // break
        container.env.forEach((env) => {
          let customenvcontainer = module.exports.setEnvVarAsObject(env);
          // if key exist in global override value
          const index = globalenv
            .map((e) => e.key)
            .indexOf(customenvcontainer.key);
          if (index > -1) {
            // envvars allready declared in global, replace with container value (hard because it's secret!!!)
            // remember final template doesn't renderer empty envvars, so if envvars override global without value
            // you ve unset the var declared in global...
            globalenv[index].name = customenvcontainer.key;
            globalenv[index].value = customenvcontainer.value;
            globalenv[index].key =
              module.exports.getSecretCustomEnvContainerName(
                container.name,
                customenvcontainer.key
              );
            //continue
            return true;
          } else {
            // envvars container added to global
            customenvcontainer.name = customenvcontainer.key;
            customenvcontainer.key =
              module.exports.getSecretCustomEnvContainerName(
                container.name,
                customenvcontainer.key
              );
            globalenv.push(customenvcontainer);
          }
        });
        return false;
      }
  ...
  /**
   * create array of env vars detected in description document
   * global.env && containerS.env
   * @param {object} data
   * @returns {array}
   */
  setEnvVarsAsKeyValue: (data, scancontainers = true) => {
    // global.env
    let envvars = module.exports.getGlobalEnv(data).map((x) => {
      return module.exports.setEnvVarAsObject(x);
    });
    // containerS.env
    if (scancontainers) {
      module.exports.getContainers(data).forEach((container) => {
        if (container.env) {
          container.env.forEach((env) => {
            let customenvcontainer = module.exports.setEnvVarAsObject(env);
            customenvcontainer.key =
            ...
            envvars = envvars.concat(customenvcontainer);
          });
        }
      });
    }
  ...
  /**
   * convert abv access from input document
  ...
  getaccessModes: (abv, scale) => {
    // application is scalable and access unset (see kubernetes documentation about access https://kubernetes.io/docs/concepts/storage/persistent-volumes/ )
  ...
  setContainersSettings: (document) => {
    ...
    for (let item of module.exports.getContainers(
      ...
      item.env = module.exports.setEnvVarsForContainerAsKeyValue(
        document,
        item.name
      );
      // no a good practise
