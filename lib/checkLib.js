  ...
  containers: (document, displayalert = true) => {
    if (document.containers) {
      if (!Array.isArray(document.containers)) {
        mainLib.displayError(
          displayalert,
          `containers must be array - element provided:`,
          document.containers
        );
        return false;
      }
  ...
  configMaps: (document, displayalert = true) => {
    if (document.global && document.global.configmaps) {
      if (!Array.isArray(document.global.configmaps)) {
        mainLib.displayError(
          displayalert,
          `global.configmaps must be array - element provided:`,
          document.configmaps
        );
        return false;
      }
      for (let map of document.global.configmaps) {
        ...
        if (!fs.existsSync(map.srcpath)) {
          mainLib.displayError(
            displayalert,
            `global.configmaps, the configmap specify srcpath ${map.srcpath}, this file is not present on the filesystem  - element provided:`,
            map
          );
          return false;
        }
  ...
  cronJob: (document, displayalert = true) => {
    if (document.global && document.global.cronjob) {
      if (!Array.isArray(document.global.cronjob)) {
        mainLib.displayError(
          displayalert,
          `global.cronjob must be array - element provided:`,
          document.cronjob
        );
        return false;
      }
      for (let cron of document.global.cronjob) {
        ...
        if (cron.type !== "curl") {
          mainLib.displayError(
            displayalert,
            `global.cronjob, the cronjob specify type ${cron.type}, only type curl is supported at this moment - element provided:`,
            cron
          );
          return false;
        }
        ...
        if (cron.cron.replace(/ +/g, " ").split(" ").length !== 5) {
          mainLib.displayError(
            displayalert,
            `global.cronjob, the cronjob specify cron ${cron.cron}, this type is not supported, you must get 5 distinct groups (* * * * *) - see kubernetes documentation about cronjob - element provided:`,
            cron
          );
          return false;
        }
